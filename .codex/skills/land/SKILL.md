...
- `gh` CLI is authenticated for `threemanifold/symphony-test`.
...
pr_title=$(gh pr view "$pr_number" --repo threemanifold/symphony-test --json title -q .title)
pr_body=$(gh pr view "$pr_number" --repo threemanifold/symphony-test --json body -q .body)
mergeable=$(gh pr view "$pr_number" --repo threemanifold/symphony-test --json mergeable -q .mergeable)
state=$(gh pr view "$pr_number" --repo threemanifold/symphony-test --json mergeStateStatus -q .mergeStateStatus)
...
gh pr merge "$pr_number" --repo threemanifold/symphony-test --squash --delete-branch \
